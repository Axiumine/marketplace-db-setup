...
// queryable "so the table can search them". What changed is narrower: an admin now needs to *list*
...
// order — so an index on it would page the table in an order no admin can predict. `firstName`,
...
// third equality field, and adding it would make the index serve a filter combination the admin
...
// Same shape and same name as `shopOwner`'s: the two admin tables page identically, and a reader
