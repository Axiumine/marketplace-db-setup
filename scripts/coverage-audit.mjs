...
const coverageString = (src, key) => {
  const block = src.slice(src.search(/\bcoverage\s*:\s*\{/));
  const m = block.match(
    new RegExp(`\\b${key}\\s*:\\s*'([^']*)'|\\b${key}\\s*:\\s*"([^"]*)"`),
  );
...
if (!configPath)
  die(`no vitest config found (looked for ${CONFIGS.join(", ")})`);
...
const reportsDirectory =
  coverageString(config, "reportsDirectory") ?? "coverage";
const lcovPath = join(reportsDirectory, "lcov.info");
if (!existsSync(lcovPath)) {
  die(
    `${lcovPath} is missing — run \`yarn test:cov\`, which produces it before calling this script.`,
  );
}
...
if (gated.length === 0) {
  die(
    `coverage.include (${include.join(", ")}) matches no tracked file — the globs are wrong.`,
  );
}
...
const missing = [
  ...new Set([...gated.filter((f) => !reported.has(f)), ...drifted]),
].sort();
...
if (undeclared.length) {
  ...
  for (const f of undeclared) {
    console.error(
      `    ${f}${driftSet.has(f) ? "   ← no coverage.include glob matches it" : ""}`,
    );
  }
  console.error(
