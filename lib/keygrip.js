...
// signing key is an operator act: a service that could mint its own would silently re-key the fleet every
 ...
 * — same prefix, same suffixes. The holders name is here only so the script can tell an operator where
 ...
 * terminal and read off an operator screen. See `keygripFingerprint.mts` for the full argument.
 */
 ...
 *
 * Refused rather than padded or hashed: a 32-byte key derived from whatever the operator happened to
 ...
 * exactly the way ADR-034 exists to prevent. A rotation is `keygripRotate` (E01-S13), which prepends and
