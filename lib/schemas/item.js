...
// The platform used to carry 13 product-type collections, each with its own validator restating
// the same base shape, and all 13 were dropped on 2026-08-04. This replaces them with one collection
// and a taxonomy (`itemCategory`), which is what they should have been: the 13 differed in their
// *category*, not in their shape, and encoding a category as a collection name is what made adding a
// fourteenth a migration instead of a document.
// **Nothing here may presume what is sold**, and do not add a per-type collection back.
//
...
// precision, a VAT treatment and a discount model all at once — and Decimal128, the type a price
// wants, is a *rejected* write everywhere else here because it cannot survive `.lean()` into
// GraphQL. It goes in with the ordering tier, in one migration, after those questions are answered.
//
