  ...
  return {
    $jsonSchema: {
      ...
      properties: {
        ...
        waitApprov: {
          bsonType: 'bool',
          // Present and true: this account may not log in until an operator approves it. Written true
          // by `shopOwnerRegister` (a stranger signed themselves up) and by `shopOwnerUpdateStatus`
          // (an operator parked an existing account); `$unset` by the same mutation on approval, which
          // is why every reader tests presence rather than equality — the field is never `false`.
          // `shopOwnerAdd` writes nothing here on purpose: an account an operator created by hand has
