...
/**
 * Strict, and refusing rather than warning — the mode every collection here is created in.
 *
 * Exported because a `collMod` has to restate it. `collMod` leaves an omitted `validationLevel` or
 * `validationAction` as it found it, so passing neither would work today; it is passed anyway, so that
 * the migration reading `LEVEL` is the same sentence as the create migration reading `LEVEL`, and a
 * change to either word cannot leave the two describing different collections.
 */
const LEVEL = {
...
module.exports = { migrationCreation, LEVEL };
