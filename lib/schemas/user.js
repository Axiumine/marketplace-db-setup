 ...
 * addresses. The divergence is in `shopOwner.js`, with the admin table that causes it.
 */
 ...
 * 3. **No `waitApprov`.** Customers self-serve: there is no admin approval gate between
 ...
 * a customer reads their own document by `_id`, and the admin table that does exist over this
 * collection — `usersActiveTbl`, paging on the `tbl_active_registeredAt` index — reads
 ...
 * the box is not built: deterministic encryption gives equal ciphertext for equal plaintext,
