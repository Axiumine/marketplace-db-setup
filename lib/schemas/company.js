 ...
 * company is put live.
 *
 * ⚠️ **`encrypted` covers exactly two fields, and the fourteen it leaves alone are the point.**
 ...
 * and the search.
 *
 * @param publicFields       whether the storefront block exists — `publicName`, `slug`,
 *                           `description`, `published` (from `20260804010000`)
 * @param publishedRequired  whether `published` is in `required`. Defaults to `publicFields` because
 *                           that is true in every state but one: the middle step of
 *                           `20260804010000`, which widens before it backfills and narrows after.
 * @param encrypted          whether the two personal names are stored as ciphertext (from
 *                           `20260808000200`)
 */
function validatorCompany({ publicFields = false, publishedRequired = publicFields, encrypted = false } = {}) {
  const schema = {
    bsonType: 'object',
    title: 'company',
    required: [
    ...
    properties: {
      ...
      contactPerson: encrypted
        ? encryptedField('the person to contact at this company, encrypted — a natural person inside a legal record')
        : {
            bsonType: 'string',
            maxLength: 50
          },
      administrator: encrypted
        ? encryptedField('the administrator of this company, encrypted — a natural person inside a legal record')
        : {
            bsonType: 'string',
            maxLength: 50
          },
